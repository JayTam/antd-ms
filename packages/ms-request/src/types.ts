...
export interface RequestConstructorConfig<T = MsAxiosResponse>
  extends Omit<AxiosRequestConfig, 'headers'>,
    MsConfigType {
  /** 业务系统有默认header，自己添加的headers会覆盖默认header */
   ...
   * 默认 verifyKey：REDACTED，verifyHmac：REDACTED
...
export interface RequestConfig<T = MsAxiosResponse>
  ...
  /** 业务系统有默认header，自己添加的headers会覆盖默认header */
  headers?: AxiosRequestConfig['headers'];
  /** 接受四种请求前后四个阶段的拦截器，业务系统默认处理也可根据业务添加，加上泛型是为了让RequestInterceptors的泛型不再是AxiosResponse的时候此处不受影响，使得接口更加独立 */
  interceptors?: RequestInterceptors<T>;
}
