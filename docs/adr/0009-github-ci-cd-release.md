...
### D7 — npm 鉴权：GitHub OIDC Trusted Publisher + provenance

- 不存 `NPM_TOKEN`。npm 后台为 `@jaytam/*` scope 的**每个包**配 Trusted Publisher，**一条**条目（npm 约束：每包只能绑一个 publisher）：
  - `repo: JayTam/antd-ms` + `workflowFilename: publish.yml` + **environment 留空**（可选字段，留空则 npm 不校验 environment，stable/canary 两个 job 各自挂 GitHub environment `release` / `snapshot` 做部署保护即可）。
  - stable 与 canary 合进同一 workflow 文件 `publish.yml`，内部按触发条件分流为 `stable` / `canary` 两个 job（见 D3 / D5）。这是为绕开「每包一个 publisher」约束而做的合流，原 `release.yml` + `snapshot.yml` 两文件方案作废。
- Workflow `permissions: { contents: write, pull-requests: write, id-token: write }`。`pnpm changeset publish` 经 OIDC 换短期 token 发包并附 provenance 来源证据。
- **Node 版本**：Trusted publishing 要求 Node 22.14.0+ 和 npm CLI 11.5.1+，三个 workflow 统一用 `node-version: '24'`。
- `.npmrc` 无需写 `_authToken`。
- 失败回退（不在本 ADR 范围）：如 provenance/OIDC 在 npm 侧舞动成本过大，临时落 `NPM_TOKEN`（granular automation、限 scope + 限包）回退，单开 ADR-0010 过渡到 OIDC。
...
- 后续 ADR 候选：
  ...
  - ADR-0012：通知内容适配 changesets —— `trigger-notify.ts` 从 changesets `publishedPackages` outputs 取本次发布内容，弃用 `findCommitByVersion`（依赖 standard-version commit 格式）；并加失败/重发策略与多通道（企业微信 + GitHub Release notes）分发。
