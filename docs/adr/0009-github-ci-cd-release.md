...
- 文档站：`pnpm build:docs` = `dumi build`；`.dumirc.ts` `outputPath: 'dist'`；`@aws-sdk/client-s3` 为 root devDeps，被 `src/fields/MsPartUpload` 引用（非纯遗留，但与文档站无关）。
...
- **作废**：老 DevOps/S3 文档站发布链。`@aws-sdk/client-s3` 保留（`src/fields/MsPartUpload` 引用，非遗留）。`design/` 目录与 `scripts/ci-design-site.ts` 搬运脚本已删除（`build:docs` 不再调它）。
