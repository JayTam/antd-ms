...
import { mergeResourceRequestList } from './utils';
 ...
 */
export function useResourceTag(props: UseResourceTagProps) {
  const { request, form, columns } = props;

  const { resourceApiVersion } = MsConfigProvider.useConfig();

  const tagColumn = columns.find((column) => column.valueType === 'resourceTags');
  const fieldProps: any = tagColumn?.fieldProps;

  const namePath = useMemo(() => {
    const defaultNamePath = '_resourceTags';
