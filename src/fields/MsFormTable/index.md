...
formTable 不能直接使用 MsField，只能通过 MsForm 组件使用。
...
<code src="./__demo__/index-column.tsx"></code>

<code src="./__demo__/action-column.tsx"></code>

<code src="./__demo__/hide-column.tsx"></code>

<code src="./__demo__/delete-confirm.tsx"></code>

<code src="./__demo__/click-edit.tsx"></code>

<!-- <code src="./__demo__/perf-debug.tsx"></code> -->
...
| indexColumn | 序列的表格列配置，与 table 保持一致 | `ColumnType` | - |
...
| actionsRender | 自定义渲染操作按钮，设置之后 `actions` 就会失效 | `(index, fields, operation, action) => ReactNode` | - | 2.21.8 |
| actionsColumn | 操作列的表格列配置，与 table 保持一致 | `ColumnType` | - |
...
| max | 列表最大数量限制 | `number` | - |
| min | 列表最小数量限制 | `number` | - |
| delPopconfirmProps | 删除确认的 Popconfirm 组件透传属性 | `PopconfirmProps \| (fields: any, index: number) => PopconfirmProps` | - |
| tableProps | 透传 antd table 属性 | [TableProps](https://4x.ant.design/components/table-cn#table) | - |
