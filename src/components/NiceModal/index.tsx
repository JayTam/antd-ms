...
export function show<
  T extends any = any,
  C extends any = any,
  P = Partial<NiceModalArgs<React.FC<C>>>,
>(modal: React.FC<C>, args?: P): Promise<T>;

// 注释 modal: string 的类型，因为禁止使用该方法
...
export const create = <P extends {}>(
  ...
  return ({ defaultVisible, keepMounted, id, ...props }) => {
    ...
    // 当路由切换时，销毁弹窗
    useEffect(() => {
      const removeOnPopstate = args?.removeOnPopstate ?? true;
      if (removeOnPopstate) {
...
export const ModalHolder: React.FC<Record<string, unknown>> = ({
  ...
  handler.open = useCallback((args: any) => show(mid, args), [mid]);
  handler.show = handler.open;
  handler.close = useCallback(() => hide(mid), [mid]);
  handler.hide = handler.close;
