...
import { setComponent, getProps, getPropValue, baseSchemaParser, setParsers } from '../../src';

describe('getPropValue', () => {
  test('prop is array', () => {
    const Input = () => <div>input</div>;
    expect(getPropValue(['123', [{ component: Input }, <div>123</div>]])).toEqual([
      '123',
      [<Input></Input>, <div>123</div>],
    ]);
  });

  test('prop is schema', () => {
    const Input: FC<{ test: number }> = ({ test }) => <div>{test}</div>;
    expect(getPropValue({ component: Input, test: 123 })).toEqual(<Input test={123}></Input>);
  ...
  test('getProps', () => {
    expect(
      getProps({
        ...
        test: <div>test</div>,
        arr: [{ component: 'a', href: 'https://foo.com', children: 'foo.com' }, 123],
      }),
    ).toEqual({
      foo: 123,
      bar: 'string',
...
describe('baseSchemaParser', () => {
  ...
  test('component is React Node', () => {
    const Form: FC<PropsWithChildren<{ className?: string }>> = ({ className, children }) => (
