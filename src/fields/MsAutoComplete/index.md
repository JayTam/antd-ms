...
| autoSelect | 自动选择模式，当 options 变化时触发：<br>`false`：关闭自动选择 <br>`null`: 置空 <br> `first`: options 第一项存在就选中它，不存在则置空 <br> `value`: 当前选中项的 value 在 options 中存在就不重新选择，不存在则置空 | `string` \| `false` | false |
| [...AutoCompleteProps](https://4x.ant.design/components/auto-complete-cn/#API) | 继承 Antd AutoComplete |  |  |
