...
const useHashState = <S extends UrlState = UrlState>(
  initialState?: S | (() => S),
  options?: Options,
) => {
  ...
  const hash = location.hash?.startsWith('#?') ? location.hash?.replace('#?', '') : '';
  ...
  const queryFromUrl = useMemo(() => {
    return parse(hash, mergedParseOptions);
  }, [hash]);
  ...
  const setState = (s: React.SetStateAction<State>) => {
    ...
    const newHash = stringify({ ...queryFromUrl, ...newState }, mergedStringifyOptions);
