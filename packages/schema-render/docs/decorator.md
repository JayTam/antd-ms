...
export const componentDecorator = ({ children, schema }) => {
  ...
  return schema.component === 'my-descriptions' ? <Card hoverable>{node}</Card> : node;
};
```
...
```jsx | pure
import { SchemaProvider } from '@jaytam/schema-render';
...
```js | pure
import { setComponentDecorator } from '@jaytam/schema-render';
...
const App = () => {
  ...
  return (
    <SchemaProvider components={components} componentDecorator={componentDecorator}>
